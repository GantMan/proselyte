...
if len < 3:
    raise Exception(
        'Parameters missing. Please pass the path to the Keras file and path to the zip directory for storing the converted moddels')
...
if path.exists(path_to_keras_file):
    ...
    raise Exception(
        u'The keras file is missing at the path {}'.format(path_to_keras_file))

# Check if the location exists
if not path.exists(path_to_zip_folder):
    raise Exception(u'The directory to the zip folder is missing at the path {}'.format(
        path_to_zip_folder))

# Keras model file is available in the variable path_of_keras_file and the path to zip directory in path_to_zip_folder

# path of the Keras model file
keras_model = './path/to/load/the/keras/model/file.h5'
# path of the Tensorflow JS directory
path_of_tfjs_dir = '.path/to/save/the/tensorflowjs_file'
# path of the Tensorflow LITE directory
path_of_tflite_dir = './path/to/tensorflowjs_file'
# path of the Tensorflow directory
path_of_tf_dir = '.path/to/save/the/tensorflow_file'
...
def keras2tfjs(path_of_keras_file, path_of_tfjs_dir):
    ...
    tf_js = converters.dispatch_keras_h5_to_tfjs_layers_model_conversion(
        path_of_keras_file, path_of_tfjs_dir)
    return tf_js
